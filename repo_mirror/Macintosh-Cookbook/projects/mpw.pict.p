program readPICT;
  {From hundredrabbits/Macintosh-Cookbook/projects/mpw.pict.p}
  {Use instead hundredrabbits/Macintosh-Cookbook/examples/pascal/pict.open.pas, as it does not have Signal()}

  {USES MemTypes, QuickDraw, OSintf, Toolintf, Packintf;}
  USES MemTypes, ToolIntf, Packintf;
    ...
    PROCEDURE GetDrawPICTFile;     {read in a PICT FILE selected by the user}
       ...
       BEGIN
          ...
          IF reply.good THEN BEGIN
             ...
             IF myFilePos <> myEOF THEN Signal(abortPICT); {added greenonline}
          END; {IF reply.good}
       END; {GetDrawPICTFile}
	   
	   BEGIN {added greenonline}
	   
	   END.  {added greenonline}
